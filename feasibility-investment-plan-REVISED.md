...
```mermaid
graph TD
    A[Visitor Lands on Platform] --> B{User Type?}

    B -->|Job Seeker| C[Register / Sign Up]
    B -->|Employer| D[Register Company]

    C --> E[AI-Powered CV Builder]
    E --> F[Complete Profile]
    F --> G[Browse Jobs / Receive Matches]
    G --> H[One-Click Apply]
    H --> I{Upgrade to Premium?}
    I -->|Yes| J[Featured Profile $2-5/month]
    I -->|No| K[Free Tier]

    D --> L[Create Company Profile]
    L --> M[Post Jobs - Free Tier]
    M --> N{Upgrade?}
    N -->|Yes| O[Paid Postings $10-50]
    N -->|Yes| P[Featured Listings $5-20]
    N -->|No| Q[Basic Free Posts]
...
```mermaid
graph TB
    REV[Total Gross Revenue<br/>Year 1: $8K-12K]
    ...
    STREAM2 --> POST1[Featured Listings $10-20]
    STREAM2 --> POST2[Urgent Hire Badge $5-10]
    STREAM2 --> POST3[Multi-post Packages]

    STREAM3 --> PREM1[Featured Profile $2-5/mo]
    STREAM3 --> PREM2[Priority Applications $1-3]
    STREAM3 --> PREM3[Resume Review $3-5]
